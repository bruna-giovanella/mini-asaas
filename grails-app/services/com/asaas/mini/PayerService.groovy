...
class PayerService {
    ...
    public Payer save(Map params, Customer customer) {
        Payer payerValues = validatePayerParams(params)

        if (payerValues.hasErrors()) {
            throw new ValidationException("Error creating payer", payerValues.errors)
        }

        if (Payer.findByCpfCnpjAndCustomer(params.cpfCnpj, customer)) {
            payerValues.errors.rejectValue("cpfCnpj", "cpfCnpj.exists", "There is already a payer with this CPF/CNPJ for this customer")
            throw new ValidationException("Error creating payer", payerValues.errors)
        }

        if (Payer.findByEmailAndCustomer(params.email, customer)) {
            payerValues.errors.rejectValue("email", "email.exists", "There is already a payer with this email for this customer")
            throw new ValidationException("Error creating payer", payerValues.errors)
        }
    ...
    private Payer validatePayerParams(Map params) {
        Payer payer = new Payer()
